...
def save_entry(name, password, email):
    entry = Entry(name, password, email)
    messagebox.showinfo("Done", f"{entry.name}, {entry.password}, {entry.tied_mail} blba")
...
def open_adding_form():
    ...
    customtkinter.CTkButton(frame2, text="Save", width=100, command=lambda: messagebox.showinfo("test", "{name_box.get()}, {password_box.get()}, {email_box.get()}")).pack(pady=(0, 10), padx=10)
